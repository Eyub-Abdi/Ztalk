...
export function AuthModal() {
  ...
  const [formLogin, setFormLogin] = useState({ identifier: "", password: "" });
  ...
  // Handle verified email from context (set by VerifyEmail page)
  useEffect(() => {
    if (verifiedEmail && isOpen && view === "signup") {
      ...
      setEmailSent(true);
      // User will complete registration via the CompleteRegistration page
      // Clear the verified email from context
      setVerifiedEmail(null);
    }
  }, [verifiedEmail, isOpen, view, setVerifiedEmail]);

  // Handle email verification callback from email link (legacy URL params method)
  useEffect(() => {
  ...
  const [signupStep, setSignupStep] = useState<0>(0);
  ...
  const clampStep = useCallback((value: number): 0 => {
    return 0;
  }, []);
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    loginMutation.mutate(
      { identifier: formLogin.identifier, password: formLogin.password },
      {
        onSuccess: (d) => {
          setAuth(d.user, d.access, d.refresh);
          close();
          toast.show("Logged in successfully", "success");
          // Navigate after auth state has time to update
          setTimeout(() => {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      setMockLoading(true);
      await http("/api/auth/password/reset/", {
        method: "POST",
        body: JSON.stringify({
      ...
      if (error.message && error.message.trim() && error.message !== '{}' && error.message !== 'null') {
        errorMessage = error.message.trim();
      } else if (error.response?.data?.message && error.response.data.message.trim()) {
        errorMessage = error.response.data.message.trim();
      } else if (error.response?.data?.error && error.response.data.error.trim()) {
        errorMessage = error.response.data.error.trim();
      }
            ...
            {view === "login" ? (
                  ...
                  <div className="w-full">
                    <div className="mb-4">
                      <button
                        ...
                        className="flex items-center gap-2 text-sm text-gray-600 hover:text-gray-800 font-medium mb-4"
                      >
              ...
              <>
                ...
                <div className="space-y-6">
                  ...
                  {/* Removed Details and Preferences steps - user completes registration via email link */}
                </div>
              </>
            )}
          </div>
