...
export function AuthModal() {
  ...
  useEffect(() => {
    ...
    if (verified === "true" && email) {
      ...
      setFormRegister((prev) => ({ ...prev, email: decodeURIComponent(email) }));
                    ...
                    <div className="px-3">
                      ...
                      <div className="space-y-2.5 mb-3">
                        {!emailSent && (
                            ...
                            onClick={() => {
                              ...
                              requestEmailMutation.mutate(
                                { email: formRegister.email },
                                {
                                  ...
                                  onError: (err) => {
                                    toast.show(err.message || "Failed to send verification email", "error");
                                  },
                                }
                              );
                            }}
                          ...
                          >
                            {requestEmailMutation.isPending ? "Sending..." : "Send verification link"}
                            {!requestEmailMutation.isPending && <FiArrowRight />}
                          </button>
                        )}
                                  ...
                                  onClick={() => {
                                    resendEmailMutation.mutate(
                                      { email: formRegister.email },
                                      {
                                        ...
                                        onError: (err) => {
                                          toast.show(err.message || "Failed to resend", "error");
                                        },
                                      }
                                    );
                                  }}
