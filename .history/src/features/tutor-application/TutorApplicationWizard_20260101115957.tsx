...
interface TutorApplicationState {
  teaching_languages: LanguageEntry[];
...
const getInitialApplicationState = (): TutorApplicationState => ({
  teaching_languages: [{ id: 1, languageId: 0, proficiencyLevel: "" }],
...
export function TutorApplicationWizard() {
  ...
  // Debug logging to check if API is working
  console.log('🐛 Language API Debug:', {
    isLoading: isLoadingLanguages,
    error: languageError?.message,
    dataReceived: !!languageData,
    success: languageData?.success,
    totalLanguages: languageData?.data?.languages?.length || 0,
    availableLanguages: availableLanguages.length,
    proficiencyRequirements: !!proficiencyRequirements,
    rawApiData: languageData
  });
  ...
  const addLanguage = () => {
    const newId = Math.max(...form.teaching_languages.map(l => l.id), 0) + 1;
    setForm(prev => ({
      ...prev,
      teaching_languages: [
        ...prev.teaching_languages,
        { id: newId, languageId: 0, proficiencyLevel: "" }
      ]
    }));
  };
  ...
  const removeLanguage = (id: number) => {
    setForm(prev => ({
      ...prev,
      teaching_languages: prev.teaching_languages.filter(l => l.id !== id)
    }));
  };

  const updateLanguage = (id: number, field: keyof Omit<LanguageEntry, 'id'>, value: any) => {
    setForm(prev => ({
      ...prev,
      teaching_languages: prev.teaching_languages.map(l =>
  ...
  const handleSubmit = async () => {
    const validLanguages = form.teaching_languages.filter(l => l.languageId > 0 && l.proficiencyLevel);
    ...
    // Validate that selected languages are available for teaching
    if (data?.data?.languages) {
      for (const lang of validLanguages) {
        const apiLang = data.data.languages.find(l => l.id === lang.languageId);
        if (!apiLang || !apiLang.is_available_for_teaching) {
          toast.show({
            title: `${apiLang?.name || 'Selected language'} is not available for teaching`,
            status: "error",
          });
          return;
        }
  ...
  const submitDisabled = 
    form.teaching_languages.filter(l => l.languageId > 0 && l.proficiencyLevel).length === 0 ||
      ...
      <div className="card bg-gray-50 dark:bg-gray-900 p-6">
        ...
        {isLoadingLanguages ? (
          <div className="text-center py-4">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">Loading available languages...</p>
          </div>
        ) : languageError ? (
          ...
          <>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
              Only Native or C2 (Proficient) level speakers can teach a language. Language availability is limited - check spots before adding.
            </p>

            {/* Language Entries */}
            <div className="space-y-4">
              {form.teaching_languages.map((languageEntry, index) => (
                <div key={languageEntry.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-white dark:bg-gray-800">
                  <div className="flex justify-between items-start mb-4">
                    <h3 className="font-medium text-gray-900 dark:text-white">
                      Language #{index + 1}
                    </h3>
                    {form.teaching_languages.length > 1 && (
                      <button
                        onClick={() => removeLanguage(languageEntry.id)}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <select
                        value={languageEntry.languageId}
                        onChange={(e) => updateLanguage(languageEntry.id, 'languageId', parseInt(e.target.value))}
                    ...
                    <div>
                      ...
                      <select 
                        value={languageEntry.proficiencyLevel}
                        onChange={(e) => updateLanguage(languageEntry.id, 'proficiencyLevel', e.target.value)}
                        className="input w-full" 
                        disabled={!languageEntry.languageId}
                      >
                        <option value="">Select level</option>
                        {proficiencyRequirements?.allowed_levels.map((level) => (
                          <option key={level} value={level}>
                            {level.toUpperCase()} - {proficiencyRequirements.level_descriptions[level]}
                          </option>
                        ))}
                      </select>
                    </div>
            ...
            {languageData?.data.languages && (
              <div className="mt-6 space-y-3">
                <h4 className="font-medium text-gray-900 dark:text-white text-sm">Language Availability:</h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 text-sm">
                  {languageData.data.languages.map((language: any) => (
                    <div 
                      key={language.id} 
                      className={`flex items-center gap-2 px-3 py-2 rounded-lg ${
                        language.is_available_for_teaching 
              ...
              <div className="mt-6 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-sm text-blue-800 dark:text-blue-200">
                    ...
                    <p className="mt-2 text-xs">{proficiencyRequirements.note}</p>
                  </div>
                </div>
              </div>
            )}
      ...
      <div className="card bg-gray-50 dark:bg-gray-900 p-6">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <label
              htmlFor="video-requirements"
              className="text-sm text-gray-700 dark:text-gray-300"
            >
              I confirm my video meets all requirements and shows me speaking the teaching language(s) clearly
            </label>
          </div>
        </div>
      </div>
