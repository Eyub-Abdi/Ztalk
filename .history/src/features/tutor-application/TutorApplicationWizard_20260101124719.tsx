...
import { useLanguageAvailability, type LanguageAvailabilityLanguage } from "./api/useLanguageAvailability";
...
interface TutorApplicationState {
  // Store selected teaching languages by language code from the API
...
export function TutorApplicationWizard() {
  ...
  // Fetch teaching language availability from API
  const {
    data: languageAvailability,
    isLoading: isLoadingLanguages,
    error: languageError,
  } = useLanguageAvailability();

  // Debug: inspect API state in browser console
  console.log("Language availability debug", {
    isLoadingLanguages,
    languageError,
    hasData: !!languageAvailability,
    languagesCount: languageAvailability?.data?.languages?.length,
    raw: languageAvailability,
  });

  const allLanguages: LanguageAvailabilityLanguage[] =
    languageAvailability?.data?.languages ?? [];

  const availableLanguages: LanguageAvailabilityLanguage[] =
    allLanguages.filter(
      (lang) => lang.is_available_for_teaching && lang.is_active
    );
  ...
  return (
    <div className="space-y-8 pb-16">
      ...
      {/* Teaching Languages - now powered by real API data */}
      <div className="card bg-gray-50 dark:bg-gray-900 p-6">
        ...
        {languageError && !isLoadingLanguages && (
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-3 mb-4 text-sm text-red-800 dark:text-red-200">
            Could not load language availability. Please try again later.
          </div>
        )}

        {!isLoadingLanguages && !languageError && (
          <>
            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-4 flex items-start gap-3">
              ...
              <span className="text-sm text-blue-800 dark:text-blue-200">
                Only Native or C2 (Proficient) level speakers can teach a
                language. Language availability is limited &mdash; check
                available spots before adding.
              </span>
            </div>
            ...
            <div className="space-y-3">
              {allLanguages.length === 0 && (
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  No teaching languages are currently available from the API.
                </p>
              )}

              {allLanguages.map((language) => {
                const isSelectable =
                  language.is_available_for_teaching && language.is_active;
                const selected = form.teaching_languages.includes(
                  language.code
                );
                ...
                return (
                  <label
                    key={language.id}
                    className={clsx(
                      "flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 sm:gap-3 rounded-lg border px-3 py-2 cursor-pointer",
                      isSelectable
                        ? "border-gray-200 dark:border-gray-700 hover:border-brand-300 bg-white dark:bg-gray-800"
                        : "border-gray-200 dark:border-gray-700 bg-gray-50/80 dark:bg-gray-800/60 opacity-60 cursor-not-allowed"
                    )}
                  >
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400 text-right">
                      <p>
                        Active teachers: {language.active_teacher_count}
                      </p>
                    </div>
                  </label>
                );
