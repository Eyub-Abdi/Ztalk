...
export function TutorApplicationWizard() {
  ...
  // Debug logging
  console.log('🐛 Debug - Language Data:', {
    languageData,
    isLoadingLanguages,
    languageError,
    availableLanguages,
        ...
        {process.env.NODE_ENV === 'development' && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
            ...
            <div className="text-sm text-yellow-700 space-y-1">
              <p>Loading: {isLoadingLanguages ? 'Yes' : 'No'}</p>
              <p>Error: {languageError ? 'Yes - ' + languageError.message : 'No'}</p>
              <p>Data received: {languageData ? 'Yes' : 'No'}</p>
              <p>Total languages: {languageData?.data?.languages?.length || 0}</p>
              <p>Available languages: {availableLanguages.length}</p>
              <button 
                onClick={async () => {
                  try {
                    const testResponse = await fetch('https://0f06a824bddb.ngrok-free.app/api/teachers/languages/availability/', {
                      headers: {
                    ...
                    alert('Direct fetch failed - check console');
                  }
                }}
