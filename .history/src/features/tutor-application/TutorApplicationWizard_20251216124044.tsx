...
function useToast() {
  return {
    show: (options: { title: string; description?: string; status?: string }) => {
      console.log(`[${options.status}] ${options.title}: ${options.description || ''}`);
    }
  };
}
...
const TEACHING_INTEREST_OPTIONS = [
  "Music", "Sports & Fitness", "Food", "Films & TV Series", "Reading",
  "Writing", "Art", "History", "Science", "Business & Finance",
  "Medical & Healthcare", "Tech", "Pets & Animals", "Gaming", "Travel",
  "Legal Services", "Marketing", "Fashion & Beauty", "Environment & Nature",
  "Animation & Comics",
];
...
export function TutorApplicationWizard() {
  const [form, setForm] = useState<TutorApplicationState>(() => getInitialApplicationState());
  ...
  useEffect(() => {
    ...
    const payload: StoredApplication = { formSnapshot: sanitizeForStorage(form) };
    try {
  ...
  const handleSubmit = () => {
    if (form.teaching_languages.length === 0) {
      toast.show({ title: "Select at least one teaching language", status: "error" });
      return;
    }
    ...
    if (!form.about_me || !form.me_as_teacher || !form.lessons_teaching_style) {
      toast.show({ title: "Complete your tutor introduction", status: "error" });
      return;
    }
    if (form.teaching_interests.length < 5) {
      toast.show({ title: "Pick at least 5 teaching interests", status: "error" });
      return;
    }
  ...
  const toggleInterest = (interest: string) => {
    setForm((prev) => {
      ...
      if (includes) {
        return { ...prev, teaching_interests: prev.teaching_interests.filter((i) => i !== interest) };
      }
      if (prev.teaching_interests.length >= 5) return prev;
      return { ...prev, teaching_interests: [...prev.teaching_interests, interest] };
    });
  };
    ...
    <div className="space-y-8 pb-16">
      <div>
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Complete the sections below to submit your teaching application. You can leave and
          return any time—your progress is saved locally.
        </p>
      </div>
      ...
      <div className="card bg-gray-50 dark:bg-gray-900 p-6">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
          Select the language(s) you are confident teaching. Only languages at a native or C2
          level are currently accepted.
        </p>
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-4 flex items-start gap-3">
          ...
          <span className="text-sm text-blue-800 dark:text-blue-200">
            Due to the volume of applications, English submissions are paused. Other languages
            remain open.
          </span>
        </div>
      ...
      <div className="card bg-gray-50 dark:bg-gray-900 p-6">
        ...
        <div className="space-y-4">
          <div className="flex flex-wrap gap-3">
            <button className="btn btn-outline text-sm">Video instructions</button>
          ...
          <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4 flex items-start gap-3">
            ...
            <span className="text-sm text-yellow-800 dark:text-yellow-200">
              Review the requirements below carefully. Applications without a valid introduction
              video are automatically rejected.
            </span>
          </div>
          ...
          <div>
            <label htmlFor="intro-video" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
          ...
          {videoPreviewUrl && (
            <div>
              <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Video preview</p>
              <div className="border-2 border-brand-200 dark:border-brand-700 rounded-lg p-4 bg-brand-50 dark:bg-brand-900/20">
                <video src={videoPreviewUrl} controls preload="metadata" className="w-full max-h-[300px] rounded-lg bg-black">
                  <track kind="captions" />
                </video>
                <div className="flex items-center justify-between mt-3">
                  <div>
                    <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">{form.video_introduction?.name}</p>
                    {form.video_introduction && (
          ...
          <div className="space-y-2 text-sm">
            <p className="font-semibold text-gray-700 dark:text-gray-300">Your video should:</p>
            <label className="flex items-center gap-2">
              <input type="checkbox" className="w-4 h-4 rounded border-gray-300 text-brand-500" />
              <span className="text-gray-600 dark:text-gray-400">Show you speaking the teaching language</span>
            </label>
            <label className="flex items-center gap-2">
              <input type="checkbox" className="w-4 h-4 rounded border-gray-300 text-brand-500" />
              <span className="text-gray-600 dark:text-gray-400">Be recorded in landscape orientation</span>
            </label>
            <label className="flex items-center gap-2">
              <input type="checkbox" className="w-4 h-4 rounded border-gray-300 text-brand-500" />
              <span className="text-gray-600 dark:text-gray-400">Have clear audio and lighting</span>
            </label>
            <label className="flex items-center gap-2">
              <input type="checkbox" className="w-4 h-4 rounded border-gray-300 text-brand-500" />
              <span className="text-gray-600 dark:text-gray-400">Exclude personal contact details</span>
            </label>
          </div>
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            <input
              ...
              onChange={(e) => setForm((prev) => ({ ...prev, has_webcam: e.target.checked }))}
              className="w-4 h-4 rounded border-gray-300 text-brand-500 focus:ring-brand-500"
            />
            <span className="text-gray-900 dark:text-white">I have a webcam and can host video lessons.</span>
          </label>
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            <input
              ...
              onChange={(e) => setForm((prev) => ({ ...prev, video_requirements_agreed: e.target.checked }))}
              className="w-4 h-4 rounded border-gray-300 text-brand-500 focus:ring-brand-500"
            />
            <span className="text-sm text-gray-900 dark:text-white">
              I understand my video must follow these rules and may appear on Ztalk channels.
            </span>
          </label>
        </div>
      </div>
      ...
      <div className="card bg-gray-50 dark:bg-gray-900 p-6">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
          Share details that help students understand your background and lesson style.
        </p>
        <div className="space-y-4">
          <div>
            <label htmlFor="about-me" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
          ...
          <div>
            <label htmlFor="me-as-teacher" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
            ...
            <textarea
              ...
              onChange={(e) => setForm((prev) => ({ ...prev, me_as_teacher: e.target.value }))}
              maxLength={700}
              rows={4}
              className="input w-full resize-y"
            />
          ...
          <div>
            <label htmlFor="lessons-teaching-style" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
              My lessons & teaching style <span className="text-red-500">*</span>
            </label>
            <textarea
              id="lessons-teaching-style"
              placeholder="Explain how you structure lessons..."
              value={form.lessons_teaching_style}
              onChange={(e) => setForm((prev) => ({ ...prev, lessons_teaching_style: e.target.value }))}
              maxLength={700}
              rows={4}
              className="input w-full resize-y"
            />
          ...
          <div>
            <label htmlFor="teaching-materials" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
            ...
            <textarea
              ...
              onChange={(e) => setForm((prev) => ({ ...prev, teaching_materials: e.target.value }))}
              rows={3}
              className="input w-full resize-y"
            />
            <div className="flex flex-wrap gap-2 mt-2">
              {["PDF", "Slides", "Audio", "Video", "Flashcards", "Homework"].map((item) => (
                <span
                  key={item}
      ...
      <div className="card bg-gray-50 dark:bg-gray-900 p-6">
        ...
        <div className="space-y-4">
          <div>
            <label htmlFor="education-experience" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
            ...
            <textarea
              ...
              onChange={(e) => setForm((prev) => ({ ...prev, education_experience: e.target.value }))}
              rows={3}
              className="input w-full resize-y"
            />
          </div>
          ...
          <div>
            <label htmlFor="teaching-experience" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
              Teaching experience
            </label>
            <textarea
          ...
          <div>
            <label htmlFor="industry-experience" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
              Industry experience
            </label>
            <textarea
              ...
              onChange={(e) => setForm((prev) => ({ ...prev, industry_experience: e.target.value }))}
              rows={3}
              className="input w-full resize-y"
            />
          </div>
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            <input
              ...
              onChange={(e) => setForm((prev) => ({ ...prev, profile_visibility: e.target.checked ? "public" : "private" }))}
              className="w-4 h-4 rounded border-gray-300 text-brand-500 focus:ring-brand-500"
            />
            <span className="text-gray-900 dark:text-white">Make my profile public once approved.</span>
          </label>
        </div>
      </div>
      ...
      <div className="card bg-gray-50 dark:bg-gray-900 p-6">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
          Pick up to five topics you love discussing. This helps match you with new students.
        </p>
        <div className="flex flex-wrap gap-3">
