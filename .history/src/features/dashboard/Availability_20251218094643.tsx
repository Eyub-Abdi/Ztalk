...
const HOURS = Array.from({ length: 18 }, (_, i) => i + 6); // 6am to 11pm (23:00)
...
export default function Availability() {
  ...
  useEffect(() => {
    if (scrollContainerRef.current) {
      ...
      if (hourIndex >= 0 && hourIndex < HOURS.length) {
        // Scroll to position the current time near the top with some padding
        const scrollPosition = Math.max(0, hourIndex * HOUR_HEIGHT - 100);
  ...
  const currentTimePosition = useMemo(() => {
    ...
    return hourIndex * HOUR_HEIGHT + minuteOffset;
  }, [currentTime]);
