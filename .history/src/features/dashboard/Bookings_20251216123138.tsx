...
const statusColors: Record<string, { bg: string; text: string }> = {
  pending: { bg: "bg-orange-100 dark:bg-orange-900", text: "text-orange-800 dark:text-orange-200" },
  awaiting_confirmation: { bg: "bg-yellow-100 dark:bg-yellow-900", text: "text-yellow-800 dark:text-yellow-200" },
  reschedule_requested: { bg: "bg-purple-100 dark:bg-purple-900", text: "text-purple-800 dark:text-purple-200" },
  confirmed: { bg: "bg-green-100 dark:bg-green-900", text: "text-green-800 dark:text-green-200" },
  scheduled: { bg: "bg-green-100 dark:bg-green-900", text: "text-green-800 dark:text-green-200" },
  in_progress: { bg: "bg-blue-100 dark:bg-blue-900", text: "text-blue-800 dark:text-blue-200" },
  completed: { bg: "bg-gray-100 dark:bg-gray-700", text: "text-gray-800 dark:text-gray-200" },
  cancelled: { bg: "bg-red-100 dark:bg-red-900", text: "text-red-800 dark:text-red-200" },
  no_show: { bg: "bg-red-100 dark:bg-red-900", text: "text-red-800 dark:text-red-200" },
...
export default function Bookings() {
  ...
  const [selectedBooking, setSelectedBooking] = useState<LessonSummary | null>(null);
  ...
  const filteredBookings = useMemo(() => {
    ...
    const sorted = [...bookings].sort((a, b) => {
      const diff = Date.parse(a.scheduled_start) - Date.parse(b.scheduled_start);
  ...
  }, [handleCloseModal, newEnd, newStart, reason, rescheduleAsync, selectedBooking, toast]);
  ...
  const nextLessonText = stats.nextLesson
    ? `${formatDateTime.format(new Date((stats.nextLesson as LessonSummary).scheduled_start))}`
    : "No upcoming lessons";
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <div className="card p-6 transition-all hover:shadow-lg hover:-translate-y-0.5">
            <div className="space-y-3">
              ...
              <p className="text-4xl font-bold text-gray-900 dark:text-white">{stats.upcoming}</p>
              <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">{nextLessonText}</p>
              <p className="text-xs text-brand-500 font-semibold">{nextLessonRelative}</p>
            </div>
          </div>
          ...
          <div className="card p-6 transition-all hover:shadow-lg hover:-translate-y-0.5">
            <div className="space-y-3">
              ...
              <p className="text-4xl font-bold text-gray-900 dark:text-white">{stats.attention}</p>
          ...
          <div className="card p-6 transition-all hover:shadow-lg hover:-translate-y-0.5">
            <div className="space-y-3">
              ...
              <p className="text-4xl font-bold text-gray-900 dark:text-white">{stats.completedThisWeek}</p>
              <p className="text-sm text-gray-500 dark:text-gray-400">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tr>
                  <td colSpan={4} className="px-6 py-12 text-center">
                    <div className="space-y-2">
                      ...
                      <p className="font-medium text-gray-900 dark:text-white">No bookings to show</p>
                      <p className="text-sm text-gray-500 dark:text-gray-400">
              ...
              {!isLoading &&
                filteredBookings.map((booking) => {
                  ...
                  const canComplete = !["completed", "cancelled", "pending"].includes(statusKey);
                  const isStarting = startLesson.isPending && startLesson.variables === booking.id;
                  const isCompleting = endLesson.isPending && endLesson.variables === booking.id;
                      ...
                      <td className="px-6 py-4">
                        <div className="space-y-1">
                          <p className="font-medium text-sm text-gray-900 dark:text-white">
                            {formatDateTime.format(new Date(booking.scheduled_start))}
                          </p>
          ...
          <div className="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-lg w-full mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-5">
                ...
                <div className="bg-brand-50 dark:bg-brand-900/30 p-4 rounded-lg">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    ...
                    <span className="font-medium">
                      {formatDateTime.format(new Date(selectedBooking.scheduled_start))}
                    </span>
                  </p>
                </div>
              )}
              ...
              <div className="space-y-4">
                <div>
                  <label htmlFor="reschedule-start" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                    New start time
                  </label>
                  <input
                    id="reschedule-start"
                ...
                <div>
                  <label htmlFor="reschedule-end" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                    New end time
                  </label>
                  <input
                    id="reschedule-end"
                ...
                <div>
                  <label htmlFor="reschedule-reason" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                    Reason (optional)
                  </label>
                  <textarea
