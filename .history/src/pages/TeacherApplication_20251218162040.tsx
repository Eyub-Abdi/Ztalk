...
} from "../features/tutor-application/api/useTeachableLanguages";
...
export default function TeacherApplication() {
  ...
  const { data: teachableLanguages, isLoading: isLanguagesLoading } =
    useTeachableLanguages();
  const { data: languageAvailability } = useLanguageAvailability(
    data.languageTaught
  );

  // Load saved draft on mount
  useEffect(() => {
    if (!isDraftLoading && savedDraft && !isInitialized) {
    ...
    } else if (!isDraftLoading && !savedDraft) {
      setIsInitialized(true);
    }
  }, [savedDraft, isDraftLoading, isInitialized]);

  // Debounced save - save draft when data or step changes
  useEffect(() => {
  ...
  }, [activeStep, data, isInitialized, user]);
  ...
  const validateStep = (step: number): boolean => {
    ...
    switch (step) {
      ...
      case 1:
        ...
        if (!data.languageTaught)
          newErrors.languageTaught = "Please select a language to teach";
        if (data.languageTaught && languageAvailability && !languageAvailability.available)
          newErrors.languageTaught = languageAvailability.message || "This language is not available for teaching";
      ...
      case 2:
        ...
        if (data.certifications.length === 0)
          newErrors.certifications = "Please upload at least one certificate";
        break;
      case 3:
        if (!data.videoFile)
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400">
                        Add languages you want to teach (Native or C2 level required)
                      </p>
                    </div>
                  </div>
                  <button
                ...
                <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3 mb-4">
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-sm text-blue-700 dark:text-blue-400">
                      Only <span className="font-semibold">Native</span> or <span className="font-semibold">C2 (Proficient)</span> level speakers can teach a language. 
                      Language availability is limited - check spots before adding.
                    </p>
                  </div>
                </div>
                ...
                {data.otherLanguages.length === 0 ? (
                  <div className="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-8 text-center border-2 border-dashed border-gray-200 dark:border-gray-600">
                    ...
                    <p className="text-sm text-gray-400 mt-1">
                      Click &quot;Add Language&quot; to add languages you want to teach
                    </p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {data.otherLanguages.map((lang, index) => {
                      const isTeachableLevel = lang.level === "Native" || lang.level === "C2 (Proficient)";
                      const langAvailability = teachableLanguages?.find(l => l.name === lang.language);
                      const isAvailable = langAvailability?.available ?? true;
                      const showAvailabilityWarning = isTeachableLevel && lang.language && !isAvailable;
                          ...
                          <div className="flex items-center justify-between mb-3">
                            <div className="flex items-center gap-2">
                              ...
                              {isTeachableLevel && lang.language && isAvailable && (
                                <span className="px-2.5 py-1 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 text-xs font-semibold rounded-lg flex items-center gap-1">
                                  <FiCheckCircle className="w-3 h-3" /> Can Teach
                                </span>
                              )}
                              {showAvailabilityWarning && (
                            ...
                            <button
                              ...
                              onClick={() =>
                                updateData(
                                  "otherLanguages",
                                  data.otherLanguages.filter((_, i) => i !== index)
                                )
                              }
                            ...
                            <div>
                              ...
                              {lang.language && langAvailability && (
                                <p className={`text-xs mt-1 ${isAvailable ? "text-green-600" : "text-red-500"}`}>
                                  {isAvailable 
                                    ? `${langAvailability.spotsRemaining} teaching spots available`
                            ...
                            <div>
                              ...
                              {lang.level && !isTeachableLevel && (
                                <p className="text-xs text-amber-600 mt-1">
                                  Only Native or C2 level can teach this language
                                </p>
                              )}
                            </div>
                          </div>
                          {showAvailabilityWarning && (
                            <div className="mt-3 p-2 bg-red-50 dark:bg-red-900/20 rounded-lg">
                              <p className="text-sm text-red-600 dark:text-red-400 flex items-center gap-2">
                                <FiInfo className="w-4 h-4" />
                                {lang.language} teaching positions are currently full. Please choose another language.
                              </p>
                            </div>
                          )}
                        </div>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-gray-50 dark:bg-gray-900/50 rounded-2xl p-6 space-y-5">
              ...
              {data.educationEntries.length === 0 ? (
                <div className="text-center py-8 text-gray-500 bg-gray-50 dark:bg-gray-700/50 rounded-xl border-2 border-dashed border-gray-200 dark:border-gray-600">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        <div>
                          ...
                          <Dropdown
                            ...
                            onChange={(value) => {
                              const newEntries = [...data.educationEntries];
                              newEntries[index] = { ...entry, fromYear: value };
                              updateData("educationEntries", newEntries);
                            }}
                          />
                        </div>
                        <div>
                          ...
                          <Dropdown
                            ...
                            onChange={(value) => {
                              const newEntries = [...data.educationEntries];
                              newEntries[index] = { ...entry, toYear: value };
                              updateData("educationEntries", newEntries);
                            }}
                          />
                        </div>
            ...
            <div className="bg-gray-50 dark:bg-gray-900/50 rounded-2xl p-6 space-y-5">
              <div className="flex items-center justify-between">
                ...
                <button
                  type="button"
                  onClick={() => {
                    const newEntry: TeachingExperienceEntry = {
                      id: Date.now().toString(),
                      fromYear: "",
                      toYear: "",
                      position: "",
                    ...
                    addTeachingExperienceEntry(newEntry);
                  }}
                  className="flex items-center gap-2 px-4 py-2.5 bg-brand-500 hover:bg-brand-600 text-white font-semibold rounded-xl transition-all shadow-lg shadow-brand-500/25"
                >
                  <FaPlus className="w-3.5 h-3.5" />
              ...
              {data.teachingExperienceEntries.length === 0 ? (
                <div className="text-center py-8 text-gray-500 bg-gray-50 dark:bg-gray-700/50 rounded-xl border-2 border-dashed border-gray-200 dark:border-gray-600">
                  <FiBriefcase className="w-12 h-12 mx-auto mb-3 opacity-30" />
                  <p className="font-medium">
                    No teaching experience added yet
                  </p>
                  <p className="text-sm mt-1">
                    Click &quot;Add&quot; to add your teaching experience
                  </p>
                </div>
              ) : (
                <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                          ...
                          <Dropdown
                            ...
                            onChange={(value) => {
                              const newEntries = [
                                ...data.teachingExperienceEntries,
                              ];
                              newEntries[index] = { ...entry, fromYear: value };
                              updateData(
                                "teachingExperienceEntries",
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) => {
                            ...
                            updateData("teachingExperienceEntries", newEntries);
                          }}
            ...
            <div className="bg-gray-50 dark:bg-gray-900/50 rounded-2xl p-6 space-y-5">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    Upload your teaching certificates (PDF only)
                  </p>
                </div>
              </div>
            ...
            <div className="bg-gray-50 dark:bg-gray-900/50 rounded-2xl p-6 space-y-5">
              ...
              <p className="text-sm text-gray-500 mb-4">
                Select the teaching methods that best describe your approach
              </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-gray-50 dark:bg-gray-900/50 rounded-2xl p-5">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <p className="text-gray-600 dark:text-gray-400">
                    Teaching: <span className="font-medium text-brand-600 dark:text-brand-400">{data.languageTaught}</span>
                  </p>
                  <p className="text-gray-600 dark:text-gray-400">
