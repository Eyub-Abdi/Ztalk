...
// API function to fetch allowed countries
const fetchAllowedCountries = async () => {
  try {
    console.log("Fetching allowed countries from:", `${API_BASE_URL}/api/teachers/allowed-countries/`);
    const response = await axios.get(`${API_BASE_URL}/api/teachers/allowed-countries/`);
    console.log("Full axios response:", response);
    console.log("Response data:", response.data);
    
    if (response.data?.success && response.data?.data?.allowed_countries) {
      console.log("Allowed countries:", response.data.data.allowed_countries);
...
const useAllowedCountries = () => {
  return useQuery({
    ...
    retry: 3,
    onError: (error) => {
      console.error("Query error for allowed countries:", error);
    },
    onSuccess: (data) => {
      console.log("Successfully fetched allowed countries:", data);
    },
  });
};
...
export default function TeacherApplication() {
  ...
  const { data: allowedCountries, isLoading: isLoadingCountries, error: countriesError } = useAllowedCountries();
  ...
  const allowedCountryOptions = (allowedCountries || []).map((c: any) => ({
    value: c.name,
    label: c.name,
    code: c.code,
    flag: c.flag_emoji
  ...
  // Country options for 'livingIn' (all countries)
  ...
  const renderCountryOption = (opt: { value: string; label: string; code?: string; flag?: string }) => {
    // For allowed countries (with code and flag from API)
    if (opt.code && opt.flag) {
      return (
        <div className="flex items-center gap-2">
  ...
  const renderSelectedCountry = (opt: { value: string; label: string; code?: string; flag?: string }) => {
    // For allowed countries (with code and flag from API)
    if (opt.code && opt.flag) {
      return (
        <div className="flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mb-2">
                    Teacher applications are only accepted from certain countries
                  </p>
                  {isLoadingCountries ? (
                    ...
                    <div className="w-full px-4 py-2.5 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-600 rounded-xl">
                      <p className="text-red-600 text-sm">Failed to load countries. Please refresh the page.</p>
                    </div>
                    ...
                    <div className="w-full px-4 py-2.5 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-600 rounded-xl">
                      <p className="text-yellow-600 text-sm">No allowed countries found. Please contact support.</p>
                    </div>
                  ) : (
                    <Dropdown
