        ...
        // Validate languages taught - need at least one language at Native/C2 level that's available
        {
          const teachableLanguagesFromList = data.otherLanguages.filter(
            (lang) =>
              lang.language &&
              lang.level &&
              (lang.level === "Native" || lang.level === "C2 (Proficient)")
          );
          const hasUnavailableLanguage = teachableLanguagesFromList.some((lang) => {
            const availability = teachableLanguages?.find((l) => l.name === lang.language);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-gray-50 dark:bg-gray-900/50 rounded-2xl p-5">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  {data.otherLanguages.filter(l => l.level === "Native" || l.level === "C2 (Proficient)").length > 0 && (
                    <div className="text-gray-600 dark:text-gray-400">
                      <span>Teaching: </span>
                      {data.otherLanguages
                        .filter(l => l.level === "Native" || l.level === "C2 (Proficient)")
                        .map((l, i) => (
                          <span key={i} className="font-medium text-brand-600 dark:text-brand-400">
                            {l.language}{i < data.otherLanguages.filter(x => x.level === "Native" || x.level === "C2 (Proficient)").length - 1 ? ", " : ""}
                          </span>
                        ))
                      }
                    </div>
