...
const STORAGE_KEY = "teacher_application_draft";

// Helper to load saved application data
const loadSavedData = (): { step: number; data: Partial<TeacherApplicationData> } | null => {
  try {
...
// Helper to save application data (excludes File objects which can't be serialized)
const saveApplicationData = (step: number, data: TeacherApplicationData) => {
  try {
    const dataToSave = {
      step,
      data: {
...
// Helper to clear saved application data
const clearSavedData = () => {
  try {
    localStorage.removeItem(STORAGE_KEY);
  } catch (e) {
    console.error("Failed to clear saved application:", e);
  }
};

export default function TeacherApplication() {
  const savedApplication = loadSavedData();
  const [activeStep, setActiveStep] = useState(savedApplication?.step ?? 0);
  const [data, setData] = useState<TeacherApplicationData>(() => ({
    ...initialData,
  ...
  useEffect(() => {
    saveApplicationData(activeStep, data);
  }, [activeStep, data]);
