                ...
                <div>
                  ...
                  {isLoadingAllCountries ? (
                    ...
                    <div className="w-full px-4 py-2.5 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-600 rounded-xl">
                      <p className="text-yellow-600 text-sm">No countries found. Please contact support.</p>
                    </div>
                  ) : (
                    <Dropdown
...
const useAllCountries = () => {
  return useQuery({
    ...
    retry: 3,
    onError: (error) => {
      console.error("Query error for all countries:", error);
    },
    onSuccess: (data) => {
      console.log("Successfully fetched all countries:", data);
    },
  });
};
...
export default function TeacherApplication() {
  ...
  const allCountryOptions = (allCountries || []).map((c: any) => ({
    value: c.name,
    label: c.name,
    code: c.code,
    flagEmoji: c.flag_emoji,
