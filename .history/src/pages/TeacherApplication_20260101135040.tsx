...
export default function TeacherApplication() {
  ...
  // Languages for "Languages Taught" dropdown come exclusively from the
  // availability endpoint. We do not mix this with native languages or
  // any other source.
  ...
  // Renderer specifically for the "Languages Taught" dropdown. This uses
  // only data coming from the availability endpoint (via useTeachableLanguages)
  // and does not rely on native language mappings.
  const renderTeachableLanguageOption = (opt: {
    value: string;
    label: string;
  }) => {
    const teachableLang = teachableLanguages?.find(
      (l) => l.name === opt.value
    );

    const flagImage = teachableLang?.flagImage;
    const code = teachableLang?.code;
