...
export default function ResetPasswordConfirm() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Extract error from backend response structure
      let errorMessage = fallback;
      
      if (err?.response?.data) {
        const responseData = err.response.data;
        
        // Check if there are specific errors array/object
        if (responseData.errors) {
          if (Array.isArray(responseData.errors)) {
            errorMessage = responseData.errors[0] || fallback;
          } else if (typeof responseData.errors === 'object') {
            const firstError = Object.values(responseData.errors)[0];
            errorMessage = Array.isArray(firstError) ? firstError[0] : firstError || fallback;
          } else {
