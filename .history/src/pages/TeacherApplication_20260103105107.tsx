...
export default function TeacherApplication() {
  ...
  const submitApplication = async () => {
    ...
    try {
      ...
      // Basic info
      formData.append("teacher_type", data.teacherType);
      formData.append("first_name", data.firstName);
      formData.append("last_name", data.lastName);
      console.log("Phone number:", data.phone, "Length:", data.phone.length);
      formData.append("phone_number", data.phone);
      formData.append("date_of_birth", data.dateOfBirth);
      formData.append("gender", data.gender);

      // Location
      formData.append("country_from", fromCountry?.code || data.from);
      formData.append(
        "country_living_in",
        livingInCountry?.code || data.livingIn
      );
      formData.append("city_living_in", data.city || "Not specified");

      // Languages
      formData.append("native_language", data.nativeLanguage);
      const otherLanguages = data.otherLanguages.length > 0 
        ? data.otherLanguages.map((lang) => ({
            ...
            can_teach_beginners:
              ...
              lang.level === "c2" ||
              lang.level === "c1",
            can_teach_advanced: lang.level === "native" || lang.level === "c2",
          }))
        : [];
      console.log("Other languages being sent:", otherLanguages);
      console.log("Other languages JSON:", JSON.stringify(otherLanguages));
      formData.append("other_languages", JSON.stringify(otherLanguages));
      ...
      // Teaching Experience
      formData.append(
        "teaching_experience_entries",
        JSON.stringify(
          data.teachingExperienceEntries.map((entry) => ({
      ...
      // Certifications
      const certificates = data.certifications.length > 0
        ? data.certifications.map((cert) => ({
            ...
            expiry_year: cert.expiryYear
              ? parseInt(cert.expiryYear)
              : undefined,
            description: cert.description,
          }))
        : [];
      console.log("Certificates being sent:", certificates);
      console.log("Certificates JSON:", JSON.stringify(certificates));
      formData.append("specialty_certificates", JSON.stringify(certificates));

      // Upload certificate attachments
      data.certifications.forEach((cert, index) => {
        if (cert.attachment) {
          formData.append(`certificate_attachment_${index}`, cert.attachment);
        }
      });

      // Bio and teaching info
      formData.append("about_me", data.introText);
      formData.append("me_as_teacher", data.teacherBio);
      
      // Truncate to max 50 chars per item and debug
      console.log("Original specialties:", data.specialties);
      console.log("Original teachingStyles:", data.teachingStyles);
      const materials = data.specialties.map(item => item.substring(0, 50));
      const interests = data.teachingStyles.map(item => item.substring(0, 50));
      console.log("Truncated materials:", materials);
      console.log("Truncated interests:", interests);
      formData.append("teaching_materials", JSON.stringify(materials));
      formData.append("teaching_interests", JSON.stringify(interests));

      // Video platform - normalize google-meet to google_meet
      const platform = data.videoPlayform === "google-meet" ? "google_meet" : data.videoPlayform;
      formData.append("preferred_platform", platform);
      if (data.videoPlayform === "zoom") {
        formData.append("zoom_meeting_link", data.zoomMeetingLink);
        formData.append("zoom_personal_meeting_id", data.zoomMeetingId);
        formData.append("zoom_meeting_password", data.zoomPasscode);
      } else if (data.videoPlayform === "google-meet") {
        formData.append("google_meet_link", data.googleMeetLink);
      }

      // Agreements and confirmations
      formData.append("terms_accepted", String(data.agreedToTerms));
      formData.append("privacy_policy_accepted", String(data.agreedToTerms));
      formData.append("internet_ready", "true");
      formData.append(
        "video_webcam_available",
        String(data.webcamConfirmation)
      );
      formData.append(
        "video_agrees_to_publishing",
        String(data.youtubePublishConsent)
      );

      // Files
      if (data.profilePhoto) {
        formData.append("profile_picture", data.profilePhoto);
      }
      if (data.videoFile) {
        formData.append("introduction_video", data.videoFile);
      }
