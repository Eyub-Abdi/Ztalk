...
// API function to fetch languages (for native language selection)
const fetchLanguages = async (): Promise<LanguageItem[]> => {
  console.log("Fetching languages from:", `${API_BASE_URL}/teachers/languages/`);
  const response = await axios.get(`${API_BASE_URL}/teachers/languages/`, {
    headers: {
      "ngrok-skip-browser-warning": "true",
      Accept: "application/json",
    },
  ...
  if (Array.isArray(data)) return data as LanguageItem[];
  if (Array.isArray(data?.data?.languages)) return data.data.languages as LanguageItem[];
  if (Array.isArray(data?.languages)) return data.languages as LanguageItem[];
  throw new Error("Unexpected response when fetching languages");
};
...
export default function TeacherApplication() {
  ...
  const languageCodeMap: Record<string, string | undefined> = Object.fromEntries(
            ...
            <div className="bg-gray-50 dark:bg-gray-900/50 rounded-2xl p-6 space-y-5">
              ...
              <div className="bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-100 dark:border-gray-700">
                ...
                {isLoadingLanguages ? (
                  ...
                  <div className="w-full px-4 py-2.5 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-600 rounded-xl">
                    <p className="text-yellow-600 text-sm">No languages found. Please contact support.</p>
                  </div>
                ) : (
                  <Dropdown
                ...
                {languagesError && !isLoadingLanguages && (
                  <p className="text-xs text-gray-500 mt-2">Using a fallback list due to a network issue.</p>
                )}
