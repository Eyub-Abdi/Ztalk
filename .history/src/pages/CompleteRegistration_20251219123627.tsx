  ...
  const validateForm = (): boolean => {
    ...
    if (!form.password) {
      ...
      const userInfo = [
        ...
        form.first_name.toLowerCase(),
        form.last_name.toLowerCase(),
      ].filter(info => info && info.length > 2);

      for (const info of userInfo) {
        if (password.toLowerCase().includes(info)) {
          passwordErrors.push("Cannot contain personal information");
          break;
        }
      }
      ...
      if (/(012|123|234|345|456|567|678|789|890)/.test(password)) {
        passwordErrors.push("Cannot contain sequential numbers");
      }

      // Check for sequential letters
      if (/(abc|bcd|cde|def|efg|fgh|ghi|hij|ijk|jkl|klm|lmn|mno|nop|opq|pqr|qrs|rst|stu|tuv|uvw|vwx|wxy|xyz)/i.test(password)) {
        passwordErrors.push("Cannot contain sequential letters");
      }
                ...
                <div>
                  ...
                  {form.password && (() => {
                    const requirements = [
                      ...
                      { met: !/(.)\1{2,}/.test(form.password), text: "No repeated characters" },
                      { met: !/(012|123|234|345|456|567|678|789|890)/.test(form.password), text: "No sequential numbers" },
                      { met: !/(abc|bcd|cde|def|efg|fgh|ghi|hij|ijk|jkl|klm|lmn|mno|nop|opq|pqr|qrs|rst|stu|tuv|uvw|vwx|wxy|xyz)/i.test(form.password), text: "No sequential letters" },
                      { met: !([form.username, form.first_name, form.last_name].filter(info => info && info.length > 2).some(info => form.password.toLowerCase().includes(info.toLowerCase()))), text: "No personal information" },
                    ];
