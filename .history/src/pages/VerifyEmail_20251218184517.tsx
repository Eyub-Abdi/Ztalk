      ...
      {toastList.map((t) => (
        <div
          ...
          className={clsx(
            ...
            t.type === "success" && "bg-gradient-to-r from-green-500/95 to-emerald-600/95 text-white",
            t.type === "error" && "bg-gradient-to-r from-red-500/95 to-rose-600/95 text-white",
            t.type === "info" && "bg-gradient-to-r from-brand-500/95 to-indigo-600/95 text-white"
          )}
        >
...
export default function VerifyEmail() {
  ...
  useEffect(() => {
    console.log("useEffect - token:", token, "hasTriggeredRef:", hasTriggeredRef.current, "mutation status:", verifyMutation.status);
    
    // Only make ONE request - use ref to survive React Strict Mode double-mount
