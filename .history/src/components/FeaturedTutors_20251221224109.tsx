      ...
      <div className="relative flex flex-col gap-4 p-5 rounded-3xl bg-white border border-gray-200 shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden backdrop-blur-sm h-full">
          ...
          <div className="flex-1 min-w-0 space-y-1.5">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-2 max-w-full">
                <h3 className="text-base font-bold text-gray-900 truncate group-hover:text-brand-600 transition-colors duration-300">
                  {tutor.name}
                  {tutor.verified && (
                    <HiBadgeCheck
                      className="w-4 h-4 text-brand-500 flex-shrink-0 ml-1 group-hover:scale-110 transition-transform duration-300"
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1.5 bg-amber-50 px-2.5 py-1 rounded-xl text-xs font-semibold border border-amber-200">
        ...
        <div className="space-y-3 relative z-10">
          <div className="flex flex-wrap gap-1.5">
            {tutor.specialties.slice(0, 3).map((s) => (
              <span
                key={s}
                className="inline-flex items-center px-2.5 py-1 rounded-xl text-xs font-semibold bg-brand-50 text-brand-700 border border-brand-200 hover:bg-brand-100 transition-colors duration-200"
              >
                {s}
              </span>
            ))}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-brand-600 font-semibold">
              ...
              <span className="text-base">${tutor.pricePerLesson}</span>
              <span className="text-xs text-gray-500 font-normal">/ lesson</span>
            </div>
        ...
        <div className="flex gap-2 pt-1 relative z-10">
          <motion.button 
            ...
            whileTap={{ scale: 0.98 }}
            className="flex-1 px-3 py-2 text-xs font-semibold text-brand-600 bg-white border border-brand-200 rounded-xl hover:bg-brand-50 transition-all duration-200 hover:shadow-md"
          >
            Profile
          </motion.button>
          <motion.button 
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            className="flex-1 px-3 py-2 text-xs font-semibold text-white bg-brand-600 border border-brand-600 rounded-xl hover:bg-brand-700 transition-all duration-200 hover:shadow-lg"
          >
            Book Now
          </motion.button>
        </div>
        ...
        <div className="absolute bottom-0 left-5 right-5 h-0.5 bg-gradient-to-r from-transparent via-brand-400 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
      </div>
    </motion.div>
  );
}
